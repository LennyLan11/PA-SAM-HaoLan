...
#         im_5channel = torch.permute(im_5channel, (2, 0, 1))  # 转换为 [channels, height, width]
...
#     #     # modified version
#     #     # Handle im_ch4
#     #     if len(self.dataset["im_ch4_path"]) > 0:
#     #         im_ch4_path = self.dataset["im_ch4_path"][idx]
#     #         im_ch4 = io.imread(im_ch4_path)
#     #         im_ch4 = im_ch4[:, :, np.newaxis] if len(im_ch4.shape) == 2 else im_ch4  # Ensure 3D shape
#     #     else:
#     #         im_ch4 = np.zeros(im.shape[:2] + (1,), dtype=np.uint8)  # Fill with zeros if missing
    
#     #     # Handle im_ch5
#     #     if len(self.dataset["im_ch5_path"]) > 0:
#     #         im_ch5_path = self.dataset["im_ch5_path"][idx]
#     #         im_ch5 = io.imread(im_ch5_path)
#     #         im_ch5 = im_ch5[:, :, np.newaxis] if len(im_ch5.shape) == 2 else im_ch5  # Ensure 3D shape
#     #     else:
#     #         im_ch5 = np.zeros(im.shape[:2] + (1,), dtype=np.uint8)  # Fill with zeros if missing
...
# all above for 3/5 channel updates on 10/09/2024
